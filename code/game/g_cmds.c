...
extern team_t TeamFromString(const char* s);
extern int TeamScore(team_t team);

static void Cmd_Team_f(gentity_t* ent) {
	char s[MAX_TOKEN_CHARS];
	team_t currentTeam;
	team_t desiredTeam;
	int redTeamCount, blueTeamCount, redTeamScore, blueTeamScore;
	...
	currentTeam = ent->client->sess.sessionTeam;
	desiredTeam = TeamFromString(s);

	if (currentTeam != TEAM_SPECTATOR && desiredTeam != TEAM_SPECTATOR) {
		trap_SendServerCommand(ent - g_entities, "print \"Manual team switching is disabled.\n\"");
		return;
	}

	if (currentTeam == TEAM_SPECTATOR && desiredTeam != TEAM_SPECTATOR) {
		redTeamCount = TeamCount(-1, TEAM_RED);
		blueTeamCount = TeamCount(-1, TEAM_BLUE);

		if (redTeamCount != blueTeamCount) {
			if ((desiredTeam == TEAM_RED && redTeamCount >= blueTeamCount) ||
				(desiredTeam == TEAM_BLUE && blueTeamCount >= redTeamCount)) {
				trap_SendServerCommand(ent - g_entities, "print \"You can only join the team with the fewest humans.\n\"");
				return;
			}
		}
		else {
			redTeamScore = TeamScore(TEAM_RED);
			blueTeamScore = TeamScore(TEAM_BLUE);

			if ((desiredTeam == TEAM_RED && redTeamScore >= blueTeamScore) ||
				(desiredTeam == TEAM_BLUE && blueTeamScore >= redTeamScore)) {
				trap_SendServerCommand(ent - g_entities, "print \"Teams are even. You can only join the team with the lowest score.\n\"");
				return;
			}
		}
	}
