...
void ShuffleTeams(void) {
	...
	// Shuffle the player indices array using level.framenum as a pseudo-random seed
	for (i = 0; i < playerCount; i++) {
		j = i + (rand() % (playerCount - i));  // Use rand() for shuffling
	...
	for (i = 0; i < playerCount; i++) {
		ent = &g_entities[playerIndices[i]];
		if (i < playerCount / 2) {
			SetTeam(ent, "red");
		}
		else {
	...
	G_Printf("Teams have been shuffled.\n");
}
