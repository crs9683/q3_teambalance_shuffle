...
static void G_RunFrame(int levelTime) {
	...
	int numMissiles;
	static int lastTeamBalanceCheck = 0; // Add static variable to track the last balance check time
	static const int teamBalanceInterval = 3000; // 3000 milliseconds (3 seconds)
	...
	// Check and balance teams periodically
	if (level.time - lastTeamBalanceCheck >= teamBalanceInterval) {
		G_CheckTeamBalance();
		lastTeamBalanceCheck = level.time;
	}
}
...
// Function to initialize team balance
void G_InitTeamBalance(void) {
	// Initialization code for team balancing
	trap_Print("Team balance initialized\n");
}
...
void balanceTeams(team_t fromTeam, team_t toTeam);

void G_CheckTeamBalance(void) {
	int team1HumanCount = 0;
	int team2HumanCount = 0;
	int i;
	gclient_t* client;
	...
	for (i = 0; i < level.maxclients; i++) {
		client = &level.clients[i];
		if (client->pers.connected != CON_CONNECTED || (g_entities[i].r.svFlags & SVF_BOT)) {
			continue;
		}
	...
	// Log the human player counts for debugging
	G_Printf("Team Balance Check: Red Team Humans = %d, Blue Team Humans = %d\n", team1HumanCount, team2HumanCount);

	// Balance logic
	if (team1HumanCount > team2HumanCount + 1) {
		// Move a human player from team 1 to team 2
		for (i = 0; i < level.maxclients; i++) {
			client = &level.clients[i];
			if (client->sess.sessionTeam == TEAM_RED && !(g_entities[i].r.svFlags & SVF_BOT)) {
				G_Printf("Switching Player %d from Red to Blue\n", i);
				SetTeam(&g_entities[i], "blue");
				break;
			}
		}
	}
	else if (team2HumanCount > team1HumanCount + 1) {
		// Move a human player from team 2 to team 1
		for (i = 0; i < level.maxclients; i++) {
			client = &level.clients[i];
			if (client->sess.sessionTeam == TEAM_BLUE && !(g_entities[i].r.svFlags & SVF_BOT)) {
				G_Printf("Switching Player %d from Blue to Red\n", i);
	...
	trap_Print("Checking team balance\n");
}
...
void balanceTeams(team_t fromTeam, team_t toTeam) {
	...
	// Gather player data from the team with more players
	for (i = 0; i < level.maxclients; i++) {
		client = &level.clients[i];
		if (client->pers.connected != CON_CONNECTED) {
			continue;
		...
		if (client->sess.sessionTeam == fromTeam) {
			players[playerCount].clientNum = i;
			players[playerCount].kills = client->ps.persistant[PERS_SCORE];
			players[playerCount].timeInMatch = level.time - client->pers.enterTime;
			playerCount++;
		}
	}

	// Sort the array of players
	qsort(players, playerCount, sizeof(playerInfo_t), ComparePlayers);

	// Ensure playerCount is greater than 0 before accessing the array
	if (playerCount > 0) {
		// Select the player with the fewest kills and least time spent in the match
		playerToSwitch = players[playerCount - 1].clientNum;

		// Switch the player to the team with fewer players
		if (fromTeam == TEAM_RED) {
			SetTeam(&g_entities[playerToSwitch], "blue");
		}
		else {
			SetTeam(&g_entities[playerToSwitch], "red");
		}
	}
}

int ComparePlayers(const void* a, const void* b) {
	const playerInfo_t* playerA = (const playerInfo_t*)a;
	const playerInfo_t* playerB = (const playerInfo_t*)b;

	if (playerA->kills != playerB->kills) {
		return playerB->kills - playerA->kills;
	}
	return playerA->timeInMatch - playerB->timeInMatch;
}
